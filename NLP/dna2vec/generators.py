...
class KmerSeqIterable_laoder:
      ...
      def __iter__(self):
               ...
               for (dirpath, dirnames, filenames) in os.walk(self.path):
                    jsonfiles += [os.path.join(dirpath, file) for file in filenames]
               for jsonfile in jsonfiles:
                 try:
                   self.logger.info('loading file {}'.format(jsonfile))
                   f = open(jsonfile)
                   loadedf = json.load(f)
               #self.logger.info('bbbbbbbbbbbbbbbbbb {}'.format(type(loadedf)))
                   for i in range(len(loadedf)):
                   #for j in range(len(loadedf[i])):
                         #print(loadedf[i][j])
                       yield(loadedf[i])
                 except: 
                         print('problem loading',jsonfile)
