...
class CustomSaver(tf.keras.callbacks.Callback):
   def on_epoch_end(self, epoch, logs={}):
            #pred_val= self.model.predict_classes(X_test)
            #accuracy = accuracy_score(y_test, pred_val)
            #print('Accuracy: %f' % accuracy)
            # precision tp / (tp + fp)
            #precision = precision_score(y_test, pred_val,average='macro')
            #print('Precision: %f' % precision)
            # recall: tp / (tp + fn)
            #recall = recall_score(y_test, pred_val,average='macro')
...
dataset = pd.read_csv('/bettik/matouguib/vectorisation_results/new/reseq_mean_all/reseq_mean_all.csv')
...
model.compile(optimizer=optm,loss='categorical_crossentropy', metrics=['sparse_categorical_accuracy'])
saver = CustomSaver()

#model = tf.keras.models.load_model('model_310.h5',custom_objects={'precision':precision,'recall':recall})
